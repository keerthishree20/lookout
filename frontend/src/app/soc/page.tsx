...
import { API_URL, ApiError, api, streamUrl } from "@/lib/api";
...
const TABS = [
  ...
  { key: "honeypot", label: "Honeypot", icon: FileWarning },
...
export default function SocConsole() {
  ...
  // Only a SOC analyst gets past here.
  useEffect(() => {
    const s = loadSession();
    if (!s || s.kind !== "soc") {
      router.replace("/login?next=soc");
      return;
    }
    setAnalyst(s.profile.username);
    setReady(true);
  }, [router]);
    ...
    <div className="mx-auto w-full max-w-[1500px] px-4 pb-16 sm:px-6">
      ...
      <header className="flex flex-wrap items-center justify-between gap-4 py-5">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-zinc-500">
              SOC console · Privileged-access misuse &amp; insider-threat detection · Meridian Bank (simulated)
            </p>
          </div>
        </div>
        <div className="flex flex-wrap items-center gap-2 text-xs">
          ...
          <Button variant="ghost" onClick={signOut} title={`Signed in as ${analyst}`}>
            <LogOut className="h-4 w-4" /> {analyst}
          </Button>
        </div>
      </header>
      ...
      <main className="mt-4">
        ...
        {tab === "honeypot" && <HoneypotPanel refreshKey={refreshKey} analyst={analyst} />}
